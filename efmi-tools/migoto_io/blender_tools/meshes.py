...
def create_merged_object(context):
    ...
    merged_obj['EFMI:MergedObjectComponents'] = json.dumps(vertex_counts)
...
def transfer_position_data(context, apply_deltas_to_shapekeys = False):
    ...
    merged_object_components = merged_obj.get('EFMI:MergedObjectComponents', None)

    if merged_object_components is None:
        raise ValueError(f'Object is missing EFMI:MergedObjectComponents atribute!')
