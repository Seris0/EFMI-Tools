...
class TextureAssigner:
    @staticmethod
    def run(object_source_folder, debug=False):
        if not object_source_folder.exists():
            print(f"Object sources folder not found: {object_source_folder}")
            return

        comp_to_data = TextureAssigner._load_texture_usage(object_source_folder)
        if not comp_to_data:
            print("No mappings loaded, aborting.")
            return

        hash_to_file = TextureAssigner._build_hash_to_filepath(object_source_folder)
        component_diffuse_files = TextureAssigner._build_component_diffuse_filepaths(object_source_folder)
        ...
        for obj in meshes_to_process:
            ...
            filepath = TextureAssigner._resolve_labeled_component_diffuse(
                info['component_id'],
                component_diffuse_files,
                debug,
            )
    ...
    @staticmethod
    def _parse_components_from_filename(path):
        match = re.search(r'\bComponents-([0-9-]+)\b', path.stem, re.IGNORECASE)
        if not match:
            return set()
        return {component_id for component_id in match.group(1).split("-") if component_id}

    @staticmethod
    def _build_component_diffuse_filepaths(folder):
        component_to_paths = {}
        for path in folder.glob("*.dds"):
            if not TextureAssigner._is_diffuse_filename(path):
                continue

            for component_id in TextureAssigner._parse_components_from_filename(path):
                component_to_paths.setdefault(component_id, []).append(path)

        return component_to_paths

    @staticmethod
    def _resolve_labeled_component_diffuse(comp_id, component_diffuse_files, debug):
        paths = component_diffuse_files.get(comp_id, [])
        if not paths:
            return None

        result = max(paths, key=lambda path: path.stat().st_size)
        TextureAssigner._debug_choice(comp_id, result, "Diffuse filename", debug)
        return result

    @staticmethod
    def _is_diffuse_filename(path):
        return "diffuse" in path.stem.lower()

    @staticmethod
    def _debug_choice(comp_id, path, reason, debug):
        if debug:
            print(f"  [Component {comp_id}] {path.name} ({reason})")
    ...
    def _resolve_texture(comp_id, comp_data, hash_to_file, debug):
        ...
        diffuse_labeled = [
            ...
            if TextureAssigner._is_diffuse_filename(path)
        ]
        if diffuse_labeled:
            result = max(diffuse_labeled, key=lambda x: x[1].stat().st_size)[1]
            TextureAssigner._debug_choice(comp_id, result, "Diffuse filename", debug)
            return result

        if len(candidates) == 1:
            TextureAssigner._debug_choice(comp_id, candidates[0][1], "single ps-t0 texture", debug)
        ...
        if len(tied) == 1:
            TextureAssigner._debug_choice(comp_id, tied[0][1], "highest ps-t10..ps-t19 count", debug)
            return tied[0][1]

        srgb = [(h, path) for h, path in tied if dds_format_byte(path) == BC7_UNORM_SRGB]

        if len(srgb) == 1:
            TextureAssigner._debug_choice(comp_id, srgb[0][1], "SRGB tie-break", debug)
            return srgb[0][1]

        pool = srgb if srgb else tied
        result = max(pool, key=lambda x: x[1].stat().st_size)[1]
        TextureAssigner._debug_choice(comp_id, result, "file size tie-break", debug)
        return result
...
def assign_textures(object_source_folder, debug=False):
    TextureAssigner.run(object_source_folder, debug)
